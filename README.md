...
下方为**完整任务 GIF 动图预览**，打开 README 即可观看，无需下载。两段均保持原速、循环展示，保留接近、夹持、搬运、释放与撤离；循环不代表新的实验次数。

### 七障碍环境中的完整抓放 · 约 43 秒

![Panthera 七障碍完整抓放原速动图](docs/media/panthera_tall_preview.gif)

空手接近与携物搬运分别绕障规划。固定布局的两个预设种子均完成任务，本片为 seed 30。

### 腕部视觉驱动的抓放 · 约 41 秒

![Panthera 仿真腕部视觉驱动抓放原速动图](docs/media/panthera_visual_preview.gif)

仿真 RGB-D 定位后生成抓取与放置目标。本片选自声明批次的成功案例，任务批次为 1/2，四项故障输入均在运动前拒绝，具体条件见报告。

动图为 768×432、20 FPS 预览；原版中文视频为 1080p、50 FPS，保存在 `docs/media/`，并由可视化页面的播放器加载。全臂、夹持特写和腕部画面以原始仿真状态重建；腕部相机为仿真的眼在手上视角。

## 可视化作品集

[打开展示页面文件](docs/index.html)：同页播放两段成功抓放视频，切换六轴 PD+g / CTC 误差曲线、阻抗位移和六组负载自适应对照，并查看七障碍路径与 100 布局任务图。页面按机器人学、控制、规划、操作、感知、学习组织。

下载仓库后，用浏览器打开 `docs/index.html` 即可离线使用，视频、曲线和图片都在仓库内。曲线取自已有 CSV 的原始采样，指标使用完整实验统计，页面不重新运行仿真。
