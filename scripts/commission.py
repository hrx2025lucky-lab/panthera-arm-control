...
安全约定
--------
🔴🔴 **这台机器没有急停按钮**（客服 Q3 原文：「关节无抱闸和急停按钮，
断电只会直接掉落。无硬件急停安装接口。」）所以：

* 🔴 **手全程放在键盘 Ctrl+C 上**，直到第 6 步结束。
  出事按 Ctrl+C —— 程序退出、电机进入阻尼模式、手臂**缓慢落下**。
* ⛔ **绝不拔电源、绝不关电源开关。** 断电 = 无抱闸 = **自由落体**。
  这和大多数工业臂的直觉相反，必须刻意记住。
* 🔴 高姿态下方垫软物（泡沫/毛巾），因为"缓慢落下"仍然是落下。
* 🔴 工作空间清空，人不要站在手臂正下方或前方
* 🔴 底座必须固定牢（见客服问题 Q4）
* ⚠️ 任何一步出现异常抖动/异响 —— **立刻 Ctrl+C**，不要"再试一次看看"

⚠️ 还要记住：三个硬件保护（``motor_timeout_ms``/``pos_limit_enable``/
``tor_limit_enable``）出厂**默认全关**，官方也建议保持关闭。
⇒ **硬件层没有任何兜底，:class:`SafetyLayer` 是唯一的保护。**
"""
...
def step1_official_examples() -> None:
  ...
  按顺序执行，每个都确认正常再进行下一个：
    ...
    3) python 2_gravity_compensation_control.py
       ✓ 手臂不下垂，手推能拖动，松手停在原地
       ⚠️ 这是第一次跑纯力矩模式，手放 Ctrl+C 上（本机没有急停按钮）
...
def step5_verify_clamp(be) -> None:
  ...
  ⚠️ 只对 J1 做，且只持续很短时间。
""")
    if not confirm("确认开始（手仍在 Ctrl+C 上）？"):
        abort("用户取消")
...
def main() -> int:
    ...
    if not args.sim:
        print("\n  🔴🔴🔴 真机模式。确认：")
        print("     [ ] 底座已固定牢")
        print("     [ ] 手在 Ctrl+C 上（本机无急停按钮，绝不拔电）")
        print("     [ ] 手臂下方已垫软物")
        print("     [ ] 工作空间无人无障碍")
        print("     [ ] 已在 --sim 下演练过一遍")
