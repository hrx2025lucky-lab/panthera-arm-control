...
不发指令**。纯力矩模式下我们传 ``pos=0``，而 Leader 配置里 J2/J3 的
下限**恰好是 0.0**——余量为零。任何微小改动（换 URDF、改配置）都可能
让 ``pos=0`` 落到限位外，届时**指令被静默丢弃，手臂失去所有力矩直接落下**。
...
class SafetyLayer:
    ...
    def filter(self, tau, q, qd) -> tuple[np.ndarray, SafetyStatus]:
        ...
        # ⑥ 限幅（⚠️ 必须最后做）
        clipped = np.clip(tau, -cfg.tau_max, cfg.tau_max)
    ...
    def safe_position_argument(self, q) -> np.ndarray:
        ...
        Leader 配置里 J2/J3 的下限恰好是 0.0，余量为零，随时可能翻车。
